    ...
    with open(output_path, 'w') as f:
        ...
        for id, message in spec.messages.items():
            ...
            for segment_name, segment in message.segments.items():
                f.write('#define __' + message.name.upper() + '__' + segment_name.upper() + '__start ' + str(segment.position[0]) + '\n')
                f.write('#define __' + message.name.upper() + '__' + segment_name.upper() + '__end ' + str(segment.position[1]) + '\n')
                for value_name, value in segment.values.items():
                    if value.range[0] == value.range[1]:
                        f.write('#define ____' + message.name.upper() + '__' + segment_name.upper() + '__' + value_name + ' ' + str(value.range[0]) + '\n')
                    else:
                        f.write('#define ____' + message.name.upper() + '__' + segment_name.upper() + '__' + value_name + '__FROM ' + str(value.range[0]) + '\n')
                        f.write('#define ____' + message.name.upper() + '__' + segment_name.upper() + '__' + value_name + '__TO ' + str(value.range[1]) + '\n')
