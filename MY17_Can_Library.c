...
Can_MsgID_T Can_MsgType(void) {
  ...
  switch(id) {
    ...
    case LV_BATTERY_VOLTAGE__id:
      return Can_Lv_Battery_Voltage_Msg;
    case ACCELEROMETER_HORIZONTAL__id:
      return Can_Accelerometer_Horizontal_Msg;
    case ACCELEROMETER_VERTICAL__id:
      return Can_Accelerometer_Vertical_Msg;
    case GYRO_HORIZONTAL__id:
      return Can_Gyro_Horizontal_Msg;
    case GYRO_VERTICAL__id:
      return Can_Gyro_Vertical_Msg;
    case MAGNETOMETER_HORIZONTAL__id:
      return Can_Magnetometer_Horizontal_Msg;
    case MAGNETOMETER_VERTICAL__id:
      return Can_Magnetometer_Vertical_Msg;
    default:
...
TO_CAN(Can_FrontCanNode_DriverOutput){
  ...
  bistring = INSERT(type_in->requested_torque, bitsring, 0, 15);
  bistring = INSERT(type_in->brake_pressure, bitsring, 16, 7);
  bistring = INSERT(type_in->steering_position, bitsring, 24, 7);
  bistring = INSERT(type_in->throttle_implausible, bitsring, 32, 0);
  bistring = INSERT(type_in->brake_throttle_conflict, bitsring, 33, 0);
  bistring = INSERT(type_in->brake_engaged, bitsring, 34, 0);
  bistring = INSERT(type_in->reserved, bitsring, 35, 4);
  bistring = INSERT(type_in->torque_before_control, bitsring, 40, 15);
...
FROM_CAN(Can_FrontCanNode_DriverOutput){
  ...
  type_out->requested_torque = EXTRACT(bitstring, 0, 15);
  type_out->brake_pressure = EXTRACT(bitstring, 16, 7);
  type_out->steering_position = EXTRACT(bitstring, 24, 7);
  type_out->throttle_implausible = EXTRACT(bitstring, 32, 0);
  type_out->brake_throttle_conflict = EXTRACT(bitstring, 33, 0);
  type_out->brake_engaged = EXTRACT(bitstring, 34, 0);
  type_out->reserved = EXTRACT(bitstring, 35, 4);
  type_out->torque_before_control = EXTRACT(bitstring, 40, 15);
}

TO_CAN(Can_RearCanNode_Heartbeat){
  uint64_t bitstring = 0;
  bistring = INSERT(type_in->is_ok, bitsring, 0, 0);
  from_bitsring(&bitstring, can_out->data);
...
FROM_CAN(Can_RearCanNode_Heartbeat){
  uint64_t bitstring = 0;
  to_bitstring(can_in->data, &bitstring);
  type_out->is_ok = EXTRACT(bitstring, 0, 0);
}

TO_CAN(Can_RearCanNode_WheelSpeed){
  uint64_t bitstring = 0;
  bistring = INSERT(type_in->rear_right_wheel_speed, bitsring, 0, 31);
  bistring = INSERT(type_in->rear_left_wheel_speed, bitsring, 32, 31);
  from_bitsring(&bitstring, can_out->data);
...
FROM_CAN(Can_RearCanNode_WheelSpeed){
  uint64_t bitstring = 0;
  to_bitstring(can_in->data, &bitstring);
  type_out->rear_right_wheel_speed = EXTRACT(bitstring, 0, 31);
  type_out->rear_left_wheel_speed = EXTRACT(bitstring, 32, 31);
}

TO_CAN(Can_FrontCanNode_RawValues){
  uint64_t bitstring = 0;
  bistring = INSERT(type_in->right_throttle_pot, bitsring, 0, 9);
  bistring = INSERT(type_in->left_throttle_pot, bitsring, 10, 9);
  bistring = INSERT(type_in->front_brake_pressure, bitsring, 20, 9);
  bistring = INSERT(type_in->rear_brake_pressure, bitsring, 30, 9);
  bistring = INSERT(type_in->steering_pot, bitsring, 40, 9);
  from_bitsring(&bitstring, can_out->data);
...
FROM_CAN(Can_FrontCanNode_RawValues){
  uint64_t bitstring = 0;
  to_bitstring(can_in->data, &bitstring);
  type_out->right_throttle_pot = EXTRACT(bitstring, 0, 9);
  type_out->left_throttle_pot = EXTRACT(bitstring, 10, 9);
  type_out->front_brake_pressure = EXTRACT(bitstring, 20, 9);
  type_out->rear_brake_pressure = EXTRACT(bitstring, 30, 9);
  type_out->steering_pot = EXTRACT(bitstring, 40, 9);
}

TO_CAN(Can_FrontCanNode_WheelSpeed){
  uint64_t bitstring = 0;
  bistring = INSERT(type_in->front_right_wheel_speed, bitsring, 0, 31);
  bistring = INSERT(type_in->front_left_wheel_speed, bitsring, 32, 31);
  from_bitsring(&bitstring, can_out->data);
...
FROM_CAN(Can_FrontCanNode_WheelSpeed){
  uint64_t bitstring = 0;
  to_bitstring(can_in->data, &bitstring);
  type_out->front_right_wheel_speed = EXTRACT(bitstring, 0, 31);
  type_out->front_left_wheel_speed = EXTRACT(bitstring, 32, 31);
}

TO_CAN(Can_Vcu_BmsHeartbeat){
  uint64_t bitstring = 0;
  bistring = INSERT(type_in->state, bitsring, 0, 0);
  from_bitsring(&bitstring, can_out->data);
...
FROM_CAN(Can_Vcu_BmsHeartbeat){
  uint64_t bitstring = 0;
  to_bitstring(can_in->data, &bitstring);
  type_out->state = EXTRACT(bitstring, 0, 0);
}

TO_CAN(Can_Vcu_DashHeartbeat){
  uint64_t bitstring = 0;
  bistring = INSERT(type_in->rtd_light, bitsring, 0, 0);
  bistring = INSERT(type_in->ams_light, bitsring, 1, 0);
  bistring = INSERT(type_in->imd_light, bitsring, 2, 0);
  bistring = INSERT(type_in->hv_light, bitsring, 3, 0);
  bistring = INSERT(type_in->traction_control, bitsring, 4, 0);
  bistring = INSERT(type_in->limp_mode, bitsring, 5, 0);
  bistring = INSERT(type_in->lv_warning, bitsring, 6, 0);
  bistring = INSERT(type_in->active_aero, bitsring, 7, 0);
  bistring = INSERT(type_in->regen, bitsring, 8, 0);
  bistring = INSERT(type_in->shutdown_esd_drain, bitsring, 9, 0);
  bistring = INSERT(type_in->shutdown_bms, bitsring, 10, 0);
  bistring = INSERT(type_in->shutdown_imd, bitsring, 11, 0);
  bistring = INSERT(type_in->shutdown_bspd, bitsring, 12, 0);
  bistring = INSERT(type_in->shutdown_vcu, bitsring, 13, 0);
  bistring = INSERT(type_in->shutdown_precharge, bitsring, 14, 0);
  bistring = INSERT(type_in->shutdown_master_reset, bitsring, 15, 0);
  bistring = INSERT(type_in->shutdown_driver_reset, bitsring, 16, 0);
  bistring = INSERT(type_in->unused, bitsring, 17, 7);
  bistring = INSERT(type_in->heartbeat_front_can_node_dead, bitsring, 25, 0);
  bistring = INSERT(type_in->heartbeat_rear_can_node_dead, bitsring, 26, 0);
  bistring = INSERT(type_in->heartbeat_bms_dead, bitsring, 27, 0);
  bistring = INSERT(type_in->heartbeat_dash_dead, bitsring, 28, 0);
  bistring = INSERT(type_in->heartbeat_mc_dead, bitsring, 29, 0);
  bistring = INSERT(type_in->heartbeat_current_sensor_dead, bitsring, 30, 0);
  bistring = INSERT(type_in->tsms_off, bitsring, 31, 0);
  bistring = INSERT(type_in->reset_latch_open, bitsring, 32, 0);
  bistring = INSERT(type_in->precharge_running, bitsring, 33, 0);
  bistring = INSERT(type_in->master_reset_not_initialized, bitsring, 34, 0);
  bistring = INSERT(type_in->driver_reset_not_initialized, bitsring, 35, 0);
  bistring = INSERT(type_in->reserved, bitsring, 36, 3);
  bistring = INSERT(type_in->lv_battery_voltage, bitsring, 40, 9);
  from_bitsring(&bitstring, can_out->data);
...
FROM_CAN(Can_Vcu_DashHeartbeat){
  uint64_t bitstring = 0;
  to_bitstring(can_in->data, &bitstring);
  type_out->rtd_light = EXTRACT(bitstring, 0, 0);
  type_out->ams_light = EXTRACT(bitstring, 1, 0);
  type_out->imd_light = EXTRACT(bitstring, 2, 0);
  type_out->hv_light = EXTRACT(bitstring, 3, 0);
  type_out->traction_control = EXTRACT(bitstring, 4, 0);
  type_out->limp_mode = EXTRACT(bitstring, 5, 0);
  type_out->lv_warning = EXTRACT(bitstring, 6, 0);
  type_out->active_aero = EXTRACT(bitstring, 7, 0);
  type_out->regen = EXTRACT(bitstring, 8, 0);
  type_out->shutdown_esd_drain = EXTRACT(bitstring, 9, 0);
  type_out->shutdown_bms = EXTRACT(bitstring, 10, 0);
  type_out->shutdown_imd = EXTRACT(bitstring, 11, 0);
  type_out->shutdown_bspd = EXTRACT(bitstring, 12, 0);
  type_out->shutdown_vcu = EXTRACT(bitstring, 13, 0);
  type_out->shutdown_precharge = EXTRACT(bitstring, 14, 0);
  type_out->shutdown_master_reset = EXTRACT(bitstring, 15, 0);
  type_out->shutdown_driver_reset = EXTRACT(bitstring, 16, 0);
  type_out->unused = EXTRACT(bitstring, 17, 7);
  type_out->heartbeat_front_can_node_dead = EXTRACT(bitstring, 25, 0);
  type_out->heartbeat_rear_can_node_dead = EXTRACT(bitstring, 26, 0);
  type_out->heartbeat_bms_dead = EXTRACT(bitstring, 27, 0);
  type_out->heartbeat_dash_dead = EXTRACT(bitstring, 28, 0);
  type_out->heartbeat_mc_dead = EXTRACT(bitstring, 29, 0);
  type_out->heartbeat_current_sensor_dead = EXTRACT(bitstring, 30, 0);
  type_out->tsms_off = EXTRACT(bitstring, 31, 0);
  type_out->reset_latch_open = EXTRACT(bitstring, 32, 0);
  type_out->precharge_running = EXTRACT(bitstring, 33, 0);
  type_out->master_reset_not_initialized = EXTRACT(bitstring, 34, 0);
  type_out->driver_reset_not_initialized = EXTRACT(bitstring, 35, 0);
  type_out->reserved = EXTRACT(bitstring, 36, 3);
  type_out->lv_battery_voltage = EXTRACT(bitstring, 40, 9);
}

TO_CAN(Can_Vcu_Mc_Message){
  uint64_t bitstring = 0;
  bistring = INSERT(type_in->reg_id, bitsring, 0, 7);
  bistring = INSERT(type_in->data_1, bitsring, 8, 7);
  bistring = INSERT(type_in->data_2, bitsring, 16, 7);
  from_bitsring(&bitstring, can_out->data);
  can_out->id = VCU_MC_MESSAGE__id
  can_out->len = 3;
}

FROM_CAN(Can_Vcu_Mc_Message){
  uint64_t bitstring = 0;
  to_bitstring(can_in->data, &bitstring);
  type_out->reg_id = EXTRACT(bitstring, 0, 7);
  type_out->data_1 = EXTRACT(bitstring, 8, 7);
  type_out->data_2 = EXTRACT(bitstring, 16, 7);
}

TO_CAN(Can_Bms_Heartbeat){
  uint64_t bitstring = 0;
  bistring = INSERT(type_in->state, bitsring, 0, 2);
  bistring = INSERT(type_in->soc_percentage, bitsring, 3, 9);
  bistring = INSERT(type_in->fan_enable, bitsring, 13, 0);
  bistring = INSERT(type_in->dcdc_enable, bitsring, 14, 0);
  bistring = INSERT(type_in->dcdc_fault, bitsring, 15, 0);
  from_bitsring(&bitstring, can_out->data);
...
FROM_CAN(Can_Bms_Heartbeat){
  uint64_t bitstring = 0;
  to_bitstring(can_in->data, &bitstring);
  type_out->state = EXTRACT(bitstring, 0, 2);
  type_out->soc_percentage = EXTRACT(bitstring, 3, 9);
  type_out->fan_enable = EXTRACT(bitstring, 13, 0);
  type_out->dcdc_enable = EXTRACT(bitstring, 14, 0);
  type_out->dcdc_fault = EXTRACT(bitstring, 15, 0);
}

TO_CAN(Can_Bms_CellTemps){
  uint64_t bitstring = 0;
  bistring = INSERT(type_in->ave_cell_temp, bitsring, 0, 14);
  bistring = INSERT(type_in->min_cell_temp, bitsring, 15, 14);
  bistring = INSERT(type_in->min_cell_temp_id, bitsring, 30, 8);
  bistring = INSERT(type_in->max_cell_temp, bitsring, 39, 14);
  bistring = INSERT(type_in->max_cell_temp_id, bitsring, 54, 8);
  from_bitsring(&bitstring, can_out->data);
...
FROM_CAN(Can_Bms_CellTemps){
  uint64_t bitstring = 0;
  to_bitstring(can_in->data, &bitstring);
  type_out->ave_cell_temp = EXTRACT(bitstring, 0, 14);
  type_out->min_cell_temp = EXTRACT(bitstring, 15, 14);
  type_out->min_cell_temp_id = EXTRACT(bitstring, 30, 8);
  type_out->max_cell_temp = EXTRACT(bitstring, 39, 14);
  type_out->max_cell_temp_id = EXTRACT(bitstring, 54, 8);
}

TO_CAN(Can_Bms_PackStatus){
  uint64_t bitstring = 0;
  bistring = INSERT(type_in->pack_voltage, bitsring, 0, 8);
  bistring = INSERT(type_in->pack_current, bitsring, 9, 10);
  bistring = INSERT(type_in->ave_cell_voltage, bitsring, 20, 9);
  bistring = INSERT(type_in->min_cell_voltage, bitsring, 30, 9);
  bistring = INSERT(type_in->min_cell_voltage_id, bitsring, 40, 6);
  bistring = INSERT(type_in->max_cell_voltage, bitsring, 47, 9);
  bistring = INSERT(type_in->max_cell_voltage_id, bitsring, 57, 6);
  from_bitsring(&bitstring, can_out->data);
...
FROM_CAN(Can_Bms_PackStatus){
  uint64_t bitstring = 0;
  to_bitstring(can_in->data, &bitstring);
  type_out->pack_voltage = EXTRACT(bitstring, 0, 8);
  type_out->pack_current = EXTRACT(bitstring, 9, 10);
  type_out->ave_cell_voltage = EXTRACT(bitstring, 20, 9);
  type_out->min_cell_voltage = EXTRACT(bitstring, 30, 9);
  type_out->min_cell_voltage_id = EXTRACT(bitstring, 40, 6);
  type_out->max_cell_voltage = EXTRACT(bitstring, 47, 9);
  type_out->max_cell_voltage_id = EXTRACT(bitstring, 57, 6);
}

TO_CAN(Can_Bms_Error){
  uint64_t bitstring = 0;
  bistring = INSERT(type_in->error_type, bitsring, 0, 3);
  from_bitsring(&bitstring, can_out->data);
...
FROM_CAN(Can_Bms_Error){
  uint64_t bitstring = 0;
  to_bitstring(can_in->data, &bitstring);
  type_out->error_type = EXTRACT(bitstring, 0, 3);
}

TO_CAN(Can_Dash_Heartbeat){
  uint64_t bitstring = 0;
  bistring = INSERT(type_in->heartbeat_on, bitsring, 0, 0);
  from_bitsring(&bitstring, can_out->data);
...
FROM_CAN(Can_Dash_Heartbeat){
  uint64_t bitstring = 0;
  to_bitstring(can_in->data, &bitstring);
  type_out->heartbeat_on = EXTRACT(bitstring, 0, 0);
}

TO_CAN(Can_Dash_Request){
  uint64_t bitstring = 0;
  bistring = INSERT(type_in->request_type, bitsring, 0, 3);
  from_bitsring(&bitstring, can_out->data);
...
FROM_CAN(Can_Dash_Request){
  uint64_t bitstring = 0;
  to_bitstring(can_in->data, &bitstring);
  type_out->request_type = EXTRACT(bitstring, 0, 3);
}

TO_CAN(Can_Mc_Response){
  uint64_t bitstring = 0;
  bistring = INSERT(type_in->reg_id, bitsring, 0, 7);
  bistring = INSERT(type_in->data, bitsring, 8, 15);
  from_bitsring(&bitstring, can_out->data);
  can_out->id = MC_RESPONSE__id
  can_out->len = 3;
}

FROM_CAN(Can_Mc_Response){
  uint64_t bitstring = 0;
  to_bitstring(can_in->data, &bitstring);
  type_out->reg_id = EXTRACT(bitstring, 0, 7);
  type_out->data = EXTRACT(bitstring, 8, 15);
}

TO_CAN(Can_CurrentSensor_Current){
  uint64_t bitstring = 0;
  bistring = INSERT(type_in->pack_current, bitsring, 16, 31);
  from_bitsring(&bitstring, can_out->data);
...
FROM_CAN(Can_CurrentSensor_Current){
  uint64_t bitstring = 0;
  to_bitstring(can_in->data, &bitstring);
  type_out->pack_current = EXTRACT(bitstring, 16, 31);
}

TO_CAN(Can_CurrentSensor_Voltage){
  uint64_t bitstring = 0;
  bistring = INSERT(type_in->pack_voltage, bitsring, 16, 31);
  from_bitsring(&bitstring, can_out->data);
...
FROM_CAN(Can_CurrentSensor_Voltage){
  uint64_t bitstring = 0;
  to_bitstring(can_in->data, &bitstring);
  type_out->pack_voltage = EXTRACT(bitstring, 16, 31);
}

TO_CAN(Can_CurrentSensor_Power){
  uint64_t bitstring = 0;
  bistring = INSERT(type_in->pack_power, bitsring, 16, 31);
  from_bitsring(&bitstring, can_out->data);
...
FROM_CAN(Can_CurrentSensor_Power){
  uint64_t bitstring = 0;
  to_bitstring(can_in->data, &bitstring);
  type_out->pack_power = EXTRACT(bitstring, 16, 31);
}

TO_CAN(Can_CurrentSensor_Energy){
  uint64_t bitstring = 0;
  bistring = INSERT(type_in->pack_energy, bitsring, 16, 31);
  from_bitsring(&bitstring, can_out->data);
...
FROM_CAN(Can_CurrentSensor_Energy){
  uint64_t bitstring = 0;
  to_bitstring(can_in->data, &bitstring);
  type_out->pack_energy = EXTRACT(bitstring, 16, 31);
}

TO_CAN(Can_Lv_Battery_Voltage){
  uint64_t bitstring = 0;
  bistring = INSERT(type_in->battery_voltage, bitsring, 0, 15);
  from_bitsring(&bitstring, can_out->data);
  can_out->id = LV_BATTERY_VOLTAGE__id
  can_out->len = 2;
}

FROM_CAN(Can_Lv_Battery_Voltage){
  uint64_t bitstring = 0;
  to_bitstring(can_in->data, &bitstring);
  type_out->battery_voltage = EXTRACT(bitstring, 0, 15);
}

TO_CAN(Can_Accelerometer_Horizontal){
  uint64_t bitstring = 0;
  bistring = INSERT(type_in->x_axis, bitsring, 0, 31);
  bistring = INSERT(type_in->y_axis, bitsring, 32, 31);
  from_bitsring(&bitstring, can_out->data);
  can_out->id = ACCELEROMETER_HORIZONTAL__id
  can_out->len = 8;
}

FROM_CAN(Can_Accelerometer_Horizontal){
  uint64_t bitstring = 0;
  to_bitstring(can_in->data, &bitstring);
  type_out->x_axis = EXTRACT(bitstring, 0, 31);
  type_out->y_axis = EXTRACT(bitstring, 32, 31);
}

TO_CAN(Can_Accelerometer_Vertical){
  uint64_t bitstring = 0;
  bistring = INSERT(type_in->z_axis, bitsring, 0, 31);
  from_bitsring(&bitstring, can_out->data);
  can_out->id = ACCELEROMETER_VERTICAL__id
  can_out->len = 4;
}

FROM_CAN(Can_Accelerometer_Vertical){
  uint64_t bitstring = 0;
  to_bitstring(can_in->data, &bitstring);
  type_out->z_axis = EXTRACT(bitstring, 0, 31);
}

TO_CAN(Can_Gyro_Horizontal){
  uint64_t bitstring = 0;
  bistring = INSERT(type_in->x_axis, bitsring, 0, 31);
  bistring = INSERT(type_in->y_axis, bitsring, 32, 31);
  from_bitsring(&bitstring, can_out->data);
  can_out->id = GYRO_HORIZONTAL__id
  can_out->len = 8;
}

FROM_CAN(Can_Gyro_Horizontal){
  uint64_t bitstring = 0;
  to_bitstring(can_in->data, &bitstring);
  type_out->x_axis = EXTRACT(bitstring, 0, 31);
  type_out->y_axis = EXTRACT(bitstring, 32, 31);
...
TO_CAN(Can_Gyro_Vertical){
  uint64_t bitstring = 0;
  bistring = INSERT(type_in->z_axis, bitsring, 0, 31);
  from_bitsring(&bitstring, can_out->data);
  can_out->id = GYRO_VERTICAL__id
  can_out->len = 4;
}

FROM_CAN(Can_Gyro_Vertical){
  uint64_t bitstring = 0;
  to_bitstring(can_in->data, &bitstring);
  type_out->z_axis = EXTRACT(bitstring, 0, 31);
}

TO_CAN(Can_Magnetometer_Horizontal){
  uint64_t bitstring = 0;
  bistring = INSERT(type_in->x_axis, bitsring, 0, 31);
  bistring = INSERT(type_in->y_axis, bitsring, 32, 31);
  from_bitsring(&bitstring, can_out->data);
  can_out->id = MAGNETOMETER_HORIZONTAL__id
  can_out->len = 8;
}

FROM_CAN(Can_Magnetometer_Horizontal){
  uint64_t bitstring = 0;
  to_bitstring(can_in->data, &bitstring);
  type_out->x_axis = EXTRACT(bitstring, 0, 31);
  type_out->y_axis = EXTRACT(bitstring, 32, 31);
...
TO_CAN(Can_Magnetometer_Vertical){
  uint64_t bitstring = 0;
  bistring = INSERT(type_in->z_axis, bitsring, 0, 31);
  from_bitsring(&bitstring, can_out->data);
  can_out->id = MAGNETOMETER_VERTICAL__id
  can_out->len = 4;
}

FROM_CAN(Can_Magnetometer_Vertical){
  uint64_t bitstring = 0;
  to_bitstring(can_in->data, &bitstring);
  type_out->z_axis = EXTRACT(bitstring, 0, 31);
}
...
DEFINE(Can_Lv_Battery_Voltage)
DEFINE(Can_Accelerometer_Horizontal)
DEFINE(Can_Accelerometer_Vertical)
DEFINE(Can_Gyro_Horizontal)
DEFINE(Can_Gyro_Vertical)
DEFINE(Can_Magnetometer_Horizontal)
DEFINE(Can_Magnetometer_Vertical)
